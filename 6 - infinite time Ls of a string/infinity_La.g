infinity_La := function(Q,rho,input_str,sigma,eps)
	...
	if inverse_left(Q,rho,input_str,sigma,eps) = "Cannot Perform The Operation" then
		return 0;
	fi;
	array := [];

	if input_str[1] = '(' then
		x := la(Q,rho,input_str,sigma,eps);
